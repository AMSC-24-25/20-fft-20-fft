...
namespace sp::hwt {
    ...
    public:
        ...
        std::vector<std::vector<double>> compress(const std::vector<std::vector<double>>& input, double threshold = 4);

        // Reconstructs the original image matrix from its compressed (transformed) version
        std::vector<std::vector<double>> reconstruct(const std::vector<std::vector<double>>& input);
        ...
        std::vector<std::vector<double>> load_img_from_binary(const std::string& compressed_image_path);
        ...
        std::vector<double> invertStep(int size, const std::vector<double>& data);
        ...
        std::vector<double> haartStep(int step, const std::vector<double>& data);
    };
}
