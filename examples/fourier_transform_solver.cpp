...
void sequential_vs_parallel_fft(const std::vector<std::complex<double>>& signal) {
    ...
    const signal_processing::handlers::signal_saver::CsvSignalSaver csv_signal_saver;
...
void plot_signal_fft_and_ifft(
    ...
    // prepare the solver
    signal_processing::fft::solver::FastFourierTransform<1> solver(
        std::array{static_cast<size_t>(signal_length)}
    );
    signal_processing::fft::solver::InverseFastFourierTransform<1> inverse_solver(
        std::array{static_cast<size_t>(signal_length)}
    ...
    solver.compute(fft_signal, signal_processing::fft::solver::ComputationMode::SEQUENTIAL);
    inverse_solver.compute(
        fft_signal, inverse_fft_signal, signal_processing::fft::solver::ComputationMode::SEQUENTIAL
    );
